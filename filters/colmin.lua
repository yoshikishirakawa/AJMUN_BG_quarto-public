...
--   :::
--
-- HTML出力では、指定された列を最小幅（内容に応じた幅）に設定します。
-- ロジック:
...
local function process_table(el, target_cols)
  ...
  el = pandoc.walk_block(el, {
    Str = function(e)
      if e.text:find("<br>") then
        ...
        local s = e.text
        local last_pos = 1
        for start_pos, end_pos in s:gmatch("()<br>()") do
          if start_pos > last_pos then
            parts:insert(pandoc.Str(s:sub(last_pos, start_pos - 1)))
          end
          parts:insert(pandoc.LineBreak())
          last_pos = end_pos
        end
        if last_pos <= #s then
          parts:insert(pandoc.Str(s:sub(last_pos)))
        end
        return parts
      end
      return nil
...
function Div(div)
  if FORMAT and not (FORMAT:match("latex") or FORMAT:match("beamer") or FORMAT:match("html")) then return nil end
  ...
  if div.classes:includes("colmin") then
    ...
    local vlines = div.attributes["vlines"] == "true" -- default false
    local hlines = div.attributes["hlines"] == "true" -- default false
    local header_rows = tonumber(div.attributes["header-rows"]) or 1
    local header_cols = tonumber(div.attributes["header-cols"]) or 1
        ...
        Table = function(el) 
          ...
          elseif header_rows < current_head_rows then
            ...
            for i = 1, excess do
              ...
              table.insert(el.bodies[1].body, 1, row)
            end
          end
            ...
            local function bold_cells(rows)
              for _, row in ipairs(rows) do
                for i = 1, math.min(header_cols, #row.cells) do
                  ...
                  -- Also add a class to the cell if possible? 
                  -- Pandoc Cell attributes are supported in recent versions.
                  if cell.attributes then
                    cell.classes:insert("header-col")
                  end
                end
              end
            end
          ...
          if vlines then el.classes:insert("colmin-vlines") else el.classes:insert("colmin-no-vlines") end
          if hlines then el.classes:insert("colmin-hlines") else el.classes:insert("colmin-no-hlines") end
          
          return process_table(el, cols) 
        end
      })
    end
  end
    ...
    if el.t == "Table" then
      ...
      if next_el and (next_el.t == "Para" or next_el.t == "Plain") then
        ...
        if raw_cols then
          ...
          if cols then
            new_blocks:insert(process_table(el, cols))
            i = i + 2
            processed = true
          end
        end
